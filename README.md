...
```
yarn install
cd src/StructuredMonolithRouting
yarn install
libman restore
```
...
You can use these commands, e.g. `StructuredMonolithRouting`  gets renamed to `MyApp`:
...
```shell
LC_ALL=C find . -type f -name '*.*' -not \( -path './node_modules/*' -o -path './src/StructuredMonolithRouting/node_modules/*' -o -path './assets' \) -exec sed -i '' 's|StructuredMonolithRouting|MyApp|g' {} \;
```
...
```bash
find . -depth -name "*StructuredMonolithRouting*" | \
while IFS= read -r ent; do mv $ent ${ent%StructuredMonolithRouting*}MyApp${ent##*StructuredMonolithRouting}; done
